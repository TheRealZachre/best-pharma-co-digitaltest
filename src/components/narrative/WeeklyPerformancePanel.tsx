...
export function WeeklyPerformancePanel({
  ...
  const maxRate = Math.max(...weeks.map((w) => w.avgEngagementRate), 0.1);
  ...
  return (
    <section className="rounded-xl border border-brand-ink/10 bg-white p-6 shadow-sm">
      ...
      <p className="mt-1 text-sm text-brand-muted">
        Average engagement rate and post volume by week
      </p>
          ...
          <div key={week.start}>
            <div className="mb-1.5 flex flex-wrap items-center justify-between gap-2 text-sm">
              ...
              <span className="text-brand-muted">
                {week.postCount} posts · {formatPercent(week.avgEngagementRate)}{" "}
                avg ER · score {week.avgEngagementScore}
              </span>
            </div>
            <div className="h-2 overflow-hidden rounded-full bg-brand-off-white">
              <div
                ...
                style={{
                  width: `${(week.avgEngagementRate / maxRate) * 100}%`,
                }}
              />
            </div>
          </div>
